...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $barcodes = isset($_POST['barcodes']) ? array_filter(explode("\n", $_POST['barcodes'])) : [];
...
<head>
    ...
    <title>Генериране на баркодове</title>
...
<div class="card-header d-flex justify-content-between align-items-center">
    ...
    <form method="POST" class="no-print mb-4">
        <div class="row g-3">
            <div class="col-md-8">
                <label for="barcodes" class="form-label">Въведете баркодове (по един на ред):</label>
                <textarea class="form-control" id="barcodes" name="barcodes" rows="5" required><?= $_POST['barcodes'] ?? '' ?></textarea>
            </div>
            <div class="col-md-4 d-flex align-items-end">
                <button type="submit" class="btn btn-primary me-2">Генерирай</button>
                <button type="button" onclick="window.print()" class="btn btn-secondary">Принтирай</button>
            </div>
        </div>
    </form>

    <?php if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($barcodes)): ?>
        <div class="results">
            <?php foreach ($barcodes as $barcode): ?>
                <?php $barcodeImage = generateBarcode($barcode); ?>
                <?php if (!empty($barcode)): ?>
                    <div class="barcode-container">
                        <div class="barcode-image">
                            <?php if (!empty($barcodeImage)): ?>
                                <img src="data:image/png;base64,<?= $barcodeImage ?>" alt="Barcode">
                                <p class="barcode-number"><?= htmlspecialchars($barcode) ?></p>
                            <?php else: ?>
