...
$allowed_report_types = ['unpaid', 'star_jo_unpaid', 'taker_unpaid', 'belejka', 'samples', 'animal_objects', 'request_sample', 'unpaid_by_jo'];
...
if ($report_type !== 'animal_objects' && $report_type !== 'unpaid_by_jo' && (!isset($_GET['start_date']) || !isset($_GET['end_date']))) {
    header('HTTP/1.1 400 Bad Request');
    echo json_encode(['error' => 'Липсващи задължителни дати.']);
    exit;
}

// Validate specific_date for unpaid_by_jo report
if ($report_type === 'unpaid_by_jo' && !isset($_GET['specific_date'])) {
    header('HTTP/1.1 400 Bad Request');
    echo json_encode(['error' => 'Липсва задължителна дата.']);
    exit;
}
...
switch ($report_type) {
    ...
    case 'request_sample':
        try {
            ...
            // Add date conditions
            if ($start_date) {
                $sql .= " AND DATE(rs.date) >= :start_date";
                $params[':start_date'] = $start_date;
            }
            if ($end_date) {
                $sql .= " AND DATE(rs.date) <= :end_date";
                $params[':end_date'] = $end_date;
            }
    ...
    case 'unpaid_by_jo':
        try {
            ...
            $specific_date = isset($_GET['specific_date']) ? DateTime::createFromFormat('d.m.Y', $_GET['specific_date'])->format('Y-m-d') : date('Y-m-d');

            // Build the query to get samples and their unpaid invoices
            $sql = "WITH UnpaidInvoices AS (
                SELECT 
                    s2.star_jo,
                    GROUP_CONCAT(DISTINCT s2.faktura ORDER BY s2.data SEPARATOR '; ') as unpaid_invoices,
                    COUNT(DISTINCT s2.faktura) as invoice_count
                FROM samples s2
                WHERE s2.faktura IS NOT NULL 
                AND s2.faktura != ''
                AND (s2.plashtane IS NULL OR s2.plashtane = '')
                GROUP BY s2.star_jo
            )
            SELECT DISTINCT
                rs.id,
            ...
            LEFT JOIN samples s ON rs.star_jo = s.star_jo AND DATE(rs.date) = DATE(s.data)
            LEFT JOIN animal_milk_types amt ON rs.star_jo = amt.star_jo
            LEFT JOIN UnpaidInvoices ui ON rs.star_jo = ui.star_jo
            WHERE DATE(rs.date) = :specific_date
            ...
            if (empty($results)) {
                echo '<div class="alert alert-info">Няма намерени заявки за избраната дата.</div>';
                return;
            }
            ...
            echo '<th>Телефон</th>';
            echo '<th>Пробовземач</th>';
            echo '<th>Брой неплатени фактури</th>';
            echo '<th>Номера на неплатени фактури</th>';
            echo '</tr></thead>';
            echo '<tbody>';
            ...
            foreach ($results as $row) {
                ...
                echo '<td>' . htmlspecialchars($row['telefon'] ?? '') . '</td>';
                echo '<td>' . htmlspecialchars($row['probovzemach']) . '</td>';
                echo '<td>' . htmlspecialchars($row['invoice_count'] ?? '0') . '</td>';
                echo '<td>' . htmlspecialchars($row['unpaid_invoices'] ?? 'Няма') . '</td>';
                echo '</tr>';
            }
            ...
            <table id="reportTable" class="table table-striped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?= htmlspecialchars($row['obshtina'] ?? '') ?></td>
