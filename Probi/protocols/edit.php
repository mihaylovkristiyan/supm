...
// Get samples for the selected date (with protocol number)
try {
    $stmt = $pdo->prepare('
        SELECT s.id, s.barkod, s.vid_mliako, s.star_jo, s.nov_jo, s.protokol_nomer, ao.proizvoditel, t.ime as probovzemach_ime
        FROM samples s
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['samples'])) {
    try {
        ...
        foreach ($_POST['samples'] as $sample_id => $protocol_number) {
            // Allow empty string to clear the protocol number
            if (trim($protocol_number) === '') {
                $stmt_null = $pdo->prepare("UPDATE samples SET protokol_nomer = NULL WHERE id = ?");
                $stmt_null->execute([$sample_id]);
                $updated_count++;
            } else {
        ...
        header("Location: edit.php?date=" . urlencode($selected_date));
        exit;
    } catch (PDOException $e) {
        $pdo->rollBack();
...
<head>
    ...
    <title>Редакция на протоколи</title>
            ...
            <div class="card-body">
                ...
                <?php if (empty($samples)): ?>
                    <div class="alert alert-info">
                        Няма намерени проби за избраната дата.
                    </div>
                <?php else: ?>
                    <form method="POST">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="text" class="form-control protocol-input" 
                                                   ...
                                                   value="<?= htmlspecialchars($sample['protokol_nomer']) ?>"
                        ...
                        <div class="d-grid gap-2">
                            <button type="submit" class="btn btn-success btn-lg">Запази промените</button>
                        </div>
                    </form>
                <?php endif; ?>
            </div>
