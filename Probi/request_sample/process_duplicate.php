...
if (!isset($_POST['original_id']) || !isset($_POST['star_jo']) || !isset($_POST['date']) || !isset($_POST['taker_id'])) {
...
$original_id = filter_var($_POST['original_id'], FILTER_SANITIZE_NUMBER_INT);
...
try {
    // Insert the duplicate request
    $stmt = $pdo->prepare("INSERT INTO request_sample (star_jo, date, taker_id) VALUES (?, ?, ?)");
    $stmt->execute([$star_jo, $mysqlDate, $taker_id]);

    $_SESSION['success_message'] = 'Заявката беше успешно дублирана.';
    header('Location: list.php');
    exit;
} catch (PDOException $e) {
    $_SESSION['error_message'] = 'Грешка при дублиране на заявката: ' . $e->getMessage();
    header('Location: list.php');
    exit;
}
